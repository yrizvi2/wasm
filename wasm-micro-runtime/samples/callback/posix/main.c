...
main(int argc, char *argv[])
{
    ...
    wasm_function_inst_t addr_func = 
        wasm_runtime_lookup_function(wasm_module_inst, "addr_export"); 
    if (!addr_func) { 
        printf("Failed to find addr_export\n"); 
        return 1; 
    } 

    uint32_t temp_result[1] = {0};  // no params to addr_export         
    if (!wasm_runtime_call_wasm(exec_env, addr_func, 0, temp_result)) { 
        printf("Call to addr_export failed: %s\n", wasm_runtime_get_exception(wasm_module_inst)); 
        return 1; 
    } 

    uint32_t func_index = temp_result[0];  // this is the function table index 
    printf("Got function index from Wasm: %u\n", func_index); 

    uint32_t params[1] = { 42 }; 
    uint32_t paramCount = sizeof(params) /sizeof(uint32_t);

    if (!wasm_runtime_call_indirect(exec_env, func_index, paramCount, params)) {
